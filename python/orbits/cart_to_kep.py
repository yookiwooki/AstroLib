...
import numpy as np
from astromath.overloads import astro_arccos, astro_2norm
...
def rv2coe(rv, mu):
    ...
    r_vec = rv[0:3]
    v_vec = rv[3:6]
    r_mag = astro_2norm(r_vec)
    v_mag = astro_2norm(v_vec)

    # Angular momentum
    h_vec = np.cross(r_vec, v_vec)
    h_mag = astro_2norm(h_vec)

    # Node vector
    n_vec = np.cross(_K_vec, h_vec)  # Node vector
    n_mag = astro_2norm(n_vec)

    # Eccentricity vector
    e_vec = ((v_mag**2 - mu/r_mag)*r_vec - np.dot(r_vec, v_vec)*v_vec)/mu
    e_mag = astro_2norm(e_vec)
    ...
    if (np.abs(e_mag) > _local_eps) and (np.abs(n_mag) > _local_eps):
        argp = astro_arccos(np.dot(n_vec, e_vec)/(n_mag*e_mag))
    ...
    if (np.abs(inc) < _local_eps and np.abs(e_mag) > _local_eps):
        lonper = astro_arccos(e_vec[0]/e_mag)
        ...
        tlon = astro_arccos(r_vec[0]/r_mag)
        if (r_vec[1] < 0.0):
            tlon = 2*np.pi - tlon
    ...
    # Circular inclined case - argument of latitude
        ...
        argl = astro_arccos(np.dot(n_vec, r_vec)/(n_mag*r_mag))
    ...
    # Default case - true anomaly
        ...
        ta = astro_arccos(np.dot(e_vec,r_vec)/(e_mag*r_mag))
